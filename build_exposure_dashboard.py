...
function buildStations(seed, sid){
  ...
  const stations=[
    ...
    {k:'business',name:'Data sensitivity',icon:'shield',lines:[`App: Transaction Processing`,`Tier: ${seed.score>=8?'0 — revenue critical':'2 — internal'}`,`Data: ${seed.internet?'PAN adjacent · tokenized':'Ops metadata'}`,`Compliance: ${/payment|pay/i.test(seed.owner)?'PCI DSS 4.0 · SOX':'SOC2 Type II'}`]},
...
function corrShortLabel(name){
  const n=String(name);
  const map={
    'Asset graph':'Asset','Asset — origin & deploy':'Asset','Identity graph':'Identity','Threat intel':'Threat intel',
    'Data sensitivity':'Data sensitivity','Business context':'Data sensitivity','Build Pipeline':'Build','Runtime controls':'Runtime controls',
...
function renderCorrelationGraph(e){
  ...
  const topSlots=hasPipeline?[
    ...
    {label:'Runtime controls',station:byKey.runtime}
  ]:[
    {label:'Finding',boxCls:'is-finding',attrs:'data-finding="1" tabindex="0" role="button" aria-label="Scanner finding"'},
  ...
  const botSlots=[
    ...
    {label:'Data sensitivity',station:byKey.business}
  ];
